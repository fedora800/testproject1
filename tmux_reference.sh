...
tmux new-window -P -t ${SESSION_NAME}:$WNO_1 -n main "bash"
...
tmux new-window -P -t ${SESSION_NAME}:$WNO_2 -n top          "top -c"

# create window 3 like above and open passwd file
tmux new-window -P -t ${SESSION_NAME}:$WNO_3 -n split_panes  "vim /etc/passwd"
...
# this is required as otherwise the pane title and status bar right hostname will not show RHOST but LHOST
...
tmux new-window -P -t ${SESSION_NAME}:$WNO_4 -n 2nd-bash   "bash"
# set active window back to window 2 (ie where top command runs)
tmux select-window -t ${SESSION_NAME}:$WNO_2

# for each of the pane on each of the windows, send a command instruction
tmux list-panes -a -F '#{session_name}:#{window_index}.#{pane_index}' | while read pane; do
  tmux send-keys -t "$pane" 'source ~/.bashrc_light' C-m
done
